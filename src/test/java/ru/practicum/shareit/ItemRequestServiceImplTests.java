...
public class ItemRequestServiceImplTests {
    ...
    @Test
    public void testGetAllRequests_NullFromAndSize_ReturnsAllItemRequestDto() {
        Integer from = null;
        Integer size = null;
        User currentUser = new User();
        List<ItemRequest> requests = new ArrayList<>();
        ...
        when(requestRepository.findAllByOrderByCreatedAsc()).thenReturn(requests);
        when(requestMapper.requestToDto(request1)).thenReturn(dto1);
        when(requestMapper.requestToDto(request2)).thenReturn(dto2);

        List<ItemRequestDto> result = itemRequestService.getAllRequests(from, size, currentUser.getId());
