...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findFirstByItemInAndStartBeforeOrderByStartDesc(List<Item> items, LocalDateTime now);

    List<Booking> findFirstByItemInAndStartAfterOrderByStartAsc(List<Item> items, LocalDateTime now);

    Optional<Booking> findFirstByItemAndItem_Owner_IdAndStartBeforeOrderByStartDesc(Item item, long userId, LocalDateTime now);
